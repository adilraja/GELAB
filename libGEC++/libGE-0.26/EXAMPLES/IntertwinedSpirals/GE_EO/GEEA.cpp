...
void main_function(int argc, char **argv){
	...
	//eoEasyEA<Indi> gga(checkpoint, eval, select, transform, merge, reduce);
	eoEasyEA<Indi> gga(checkpoint, eval, select, transform, plusReplacement);
	//eoEasyEA<Indi> gga(checkpoint, eval, select, transform, reduceMergeReduce);
	// Apply it to population
	gga(pop);
	pop.sort();// Sort population
	pop[0].invalidate();
        eval(pop[0]);// Map best individual
