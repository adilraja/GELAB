...
function [retval] = ge_createChildPopulation (population)
    childPopulation=ge_initPopulation(length(population)-1);
    j=1;
    for(i=1:length(population)/2)
        [parent1, parent2]=ge_selection(population, 2);
        [child1, child2]=ge_crossover(parent1, parent2);%Apply crossover
        if(rand(1)<0.5)
