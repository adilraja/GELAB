...
function survivedPopulation = ge_replacement (parentPopulation, childPopulation)
    population=[parentPopulation childPopulation];
    size1=length(population);
    for(i=1:length(population))
      fitness(i)=population(i).fitness;
    end
    [sortedFitness, I]=sort(fitness, 'ascend');
    survivedPopulation=population(I(1:length(population)/2));
end
